...
lineParser :: ReadP (String, Integer, Integer, Integer)
lineParser = do
  who <- word
  string " can fly "
  speed <- intParser
  ...
  rest <- intParser
  string " seconds."
  return (who, speed, dur, rest)

day14 ls =
  let rules = parse ls
      scores = [score 2503 a b c | (w, a, b, c) <- rules]
  in  maximum scores

score x speed duration rest =
  let interval = duration + rest
      (whole, part) = x `divMod` interval
      runFor = part `min` duration
  in  (whole * duration + runFor) * speed
...
day14b ls =
  let rules = parse ls
      distances = [[score d a b c | (w, a, b, c) <- rules] | d <- [1..2503]]
      d' = [winner ds | ds <- distances]
      d'' = transpose d' & map sum
  in  maximum d''

winner ds =
  let furthest = maximum ds
  in  [if d == furthest then 1 else 0 | d <- ds]
