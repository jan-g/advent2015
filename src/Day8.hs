...
dequote x = dequote' $ innards x
  where
    innards x = reverse x & tail & reverse & tail
    dequote' [] = []
    dequote' ('\\':'x':a:b:rest) =
      let hex x = if '0' <= x && x <= '9' then (ord x) - (ord '0') else (ord x) - (ord 'a') + 10
          a' = hex a
          b' = hex b
          c = chr (a' * 16 + b')
      in c : (dequote' rest)
    dequote' ('\\':'\\':rest) = '\\' : (dequote' rest)
    dequote' ('\\':'"':rest) = '"' : (dequote' rest)
    dequote' (x:rest) = x : dequote' rest

day8 ls =
  let ls' = parse ls
      l1 = sum $ map length ls
      l2 = sum $ map length ls'
  in  (l1, l2, l1 - l2)
...
requote x = "\"" ++ requote' x ++ "\""
  where
    requote' "" = ""
    requote' ('\\':rest) = "\\\\" ++ requote' rest
    requote' ('\"':rest) = "\\\"" ++ requote' rest
    requote' (x:rest) = x : requote' rest
